...
This integration supports accounts on the European GWM cloud (`region: eu`), including EU countries and Israel; the Australia/New Zealand cloud (`region: aus`); and the live-tested Russia cloud (`region: rus`). Experimental mainland-China support (`region: cn`) is available for NavInfo/AutoAI and BeanTech vehicles.
