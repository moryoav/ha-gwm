...
def test_addon_metadata_declares_internal_api_and_discovery() -> None:
    ...
    assert 'version: "0.11.6"' in config
    assert 'GWM_ORA_ADDON_VERSION: "0.11.6"' in config
...
def test_addon_presentation_assets_exist() -> None:
    ...
    assert (addon_dir / "CHANGELOG.md").read_text(encoding="utf-8") == (ROOT / "CHANGELOG.md").read_text(
        encoding="utf-8"
    )
...
def test_addon_container_build_runs_for_pull_requests() -> None:
    workflow = (ROOT / ".github/workflows/addon-build.yml").read_text(encoding="utf-8")

    assert "pull_request:\n    branches: [main]" in workflow
    assert "platform: linux/amd64" in workflow
    assert "platform: linux/arm64" in workflow
    assert "platform: linux/arm/v7" in workflow
...
def test_hacs_default_repository_readiness_files_exist() -> None:
    ...
    assert manifest["version"] == "0.11.6"
